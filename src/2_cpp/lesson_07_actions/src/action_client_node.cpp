#include "lesson_07_actions/action_client_node.hpp"
...
: rclcpp::Node("lesson_07_action_client", options)
{
...
void ActionClientNode::send_goal(int32_t order)
{
  ...
  RCLCPP_INFO(get_logger(), "Result Received: %s (Status: %d)", s.c_str(), wrapped.code);
}
...
void ActionClientNode::demo_cancel()
{
  ...
  RCLCPP_INFO(get_logger(), "Final Result after cancel: Sequence=%s, Status=%d", s.c_str(), wrapped.code);
}
...
int main(int argc, char ** argv)
{
  ...
  try {
    auto node = std::make_shared<lesson_07_actions_cpp::ActionClientNode>();
