# Lesson 06 (C++) – Lifecycle Management
...
```bash
cd ~/ros2_ws_tutorial
# Rebuild ensures our new lifecycle targets are compiled
colcon build --packages-select lesson_06_lifecycle_cpp
...
**Observation:** The terminal sits silently. The node is running but unconfigured ("Waiting for manager...").
...
ros2 run lesson_06_lifecycle_cpp lesson_06_lifecycle_subscriber --ros-args \
  --params-file src/4_interfaces/lesson_interfaces/config/topics_config.yaml \
  --params-file src/4_interfaces/lesson_interfaces/config/qos_config.yaml
```
...
This package includes both **Unit Tests** (C++ GTest logic checks) and **Integration Tests** (Python-driven state machine verification).

```bash
# Run all tests for this package (Logic + Integration)
colcon test --packages-select lesson_06_lifecycle_cpp --return-code-on-test-failure --event-handlers console_direct+
```
...
for i in {1..5}; do
   echo "--- Run #$i ---"
   colcon test --packages-select lesson_06_lifecycle_cpp \
...
* **Inheritance**: Nodes inherit from `rclcpp_lifecycle::LifecycleNode` (native ROS 2 support).
* **Publisher Gating**: Uses `rclcpp_lifecycle::LifecyclePublisher`. The middleware **automatically** mutes data when inactive (no manual `if` check required).
* **Subscriber Gating**: `rclcpp` does not have a "LifecycleSubscription", so we manually gate execution using `if (enabled_)` inside the callback.
* **Interface Abstraction**: We used `get_node_parameters_interface()` in `utils.hpp` to allow our configuration logic to work on both standard `Node` and `LifecycleNode` types without code duplication.
