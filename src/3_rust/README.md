...
These lessons use `rcllibrust`, a pure Rust client that connects to the ROS 2 network via **WebSockets**.
...
## Comparison & `rustrosext`

The goal of this workspace is to demonstrate full feature parity between Rust, C++, and Python.
While `rclrs` provides the core primitives, the **[rustrosext](https://www.google.com/search?q=https://github.com/ros2-rust/rustrosext)** crate is the initiative driving high-level feature parity (Lifecycle, Actions).

| Feature | **rclrs** | **rcllibrust** |
| --- | --- | --- |
...
| **Executor** | Yes (Spin loop) | No (Manual polling/async) |
| **Parameters** | Native | Limited |
| **Actions** | Native | Via `rustrosext` |
| **Lifecycle** | Via `rustrosext` | Via `rustrosext` |

> **Trade-off**: `rcllibrust` is ideal for lightweight tools or systems that cannot install the full ROS 2 stack (DDS), but it incurs higher latency due to JSON serialization.