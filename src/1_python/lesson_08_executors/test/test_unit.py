"""
Unit tests for Lesson 08 logic and validation.
Run with: pytest src/lesson_08_executors/test/test_unit.py
"""
from lesson_08_executors.logic import StreamEvent, TelemetryStreamValidator, TelemetryGenerator
from lesson_08_executors.utils import validate_reset_max_value, validate_timer_period_s


# --- Utils Tests (Validating the code you provided) ---
...
def test_timer_period_rejects_zero():
    ...
    # Matches: "timer_period_s must be > 0.0"
    assert ">" in r.reason
...
def test_timer_period_rejects_non_numeric():
    ...
    assert r.ok is False
    # Matches: "timer_period_s must be a number"
...
def test_reset_max_value_rejects_negative():
    ...
    assert r.ok is False
    # Matches: "reset_max_value must be >= 0"
    assert ">=" in r.reason


# --- Subscriber Logic Tests (TelemetryStreamValidator) ---
...
# --- Publisher Logic Tests (TelemetryGenerator) ---

def test_generator_starts_at_zero():
    g = TelemetryGenerator()
    assert g.next_value() == 0
    assert g.next_value() == 1
    assert g.next_value() == 2