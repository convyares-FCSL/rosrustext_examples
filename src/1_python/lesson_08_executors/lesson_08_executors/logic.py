...
Contains shared logic for sequence generation and validation, independent of ROS.
"""
...
class FibonacciGenerator:
    ...
    def step(self, current_sequence: list[int]) -> int:
        """Compute the next value given the current sequence."""
        if not current_sequence:
             return 0
        if len(current_sequence) == 1:
             return 1
        return current_sequence[-1] + current_sequence[-2]
