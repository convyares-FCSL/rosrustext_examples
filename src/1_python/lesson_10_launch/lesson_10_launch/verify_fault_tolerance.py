...
import re
import os
from lesson_10_launch.verification import run_command, wait_for_condition
...
def get_publisher_pid():
    """Find the OS PID of the lifecycle publisher."""
    # This is tricky without specialized tools.
    # For Profile A: It's the 'lesson_09_composed_deployment' process.
    # For Profile B: It's the 'lesson_06_lifecycle_publisher' process.
    # We can use pgrep/ps.
    try:
        # Try finding specific executable first (Profile B)
        pid = run_command(["pgrep", "-f", "lesson_06_lifecycle_publisher"])
        if pid:
            return int(pid.split()[0])
        
        # Fallback to composed container (Profile A)
        pid = run_command(["pgrep", "-f", "lesson_09_composed_deployment"])
        if pid:
            return int(pid.split()[0])
    except:
        pass
...
def run_profile_test(launch_file, profile_name):
    ...
    launch_process = subprocess.Popen(
        ["ros2", "launch", "lesson_10_launch", launch_file],
    ...
    try:
        ...
        time.sleep(5.0) # Longer warmup to ensured active computation
        ...
        if profile_name == "Profile B":
            ...
            # Find PID of action server
            try:
                # Profile B: lesson_08_action_server
        ...
        # Cleanup any stray nodes/actions
        time.sleep(2)
        # Force cleanup just in case
        subprocess.run(["pkill", "-f", "lesson_06"], stderr=subprocess.DEVNULL)
        subprocess.run(["pkill", "-f", "lesson_08"], stderr=subprocess.DEVNULL)
        subprocess.run(["pkill", "-f", "lesson_09"], stderr=subprocess.DEVNULL)
        time.sleep(1)

def main():
    print("--- Starting Verify Fault Tolerance (A/B Test) ---")
    ...
    if res_b['factor'] < res_a['factor']:
         ...
         print("WARN: Profile B degraded same/worse than A ({:.2f}x vs {:.2f}x). CPU saturation active.".format(res_b['factor'], res_a['factor']))
