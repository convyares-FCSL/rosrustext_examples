...
def generate_test_description():
    dut_node = LaunchNode(
        package='lesson_07_actions',
        executable='lesson_07_action_server',
        name='lesson_07_action_server',
        output='screen',
        emulate_tty=True,
...
class TestActionBlocking(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client_get_state = self.node.create_client(GetState, '/lesson_07_action_server/get_state')
        self.client_change_state = self.node.create_client(ChangeState, '/lesson_07_action_server/change_state')
        self._action_client = ActionClient(self.node, Fibonacci, '/lesson_07_action_server/fibonacci')
    ...
    def test_blocking_behavior(self):
        """
        Verify that executing an action BLOCKS the lifecycle services (Lesson 07 Failure Mode).
        """
        ...
        if future.done():
            self.fail("Lifecycle GetState service responded immediately! The node is NOT blocking as expected for Lesson 07.")
        else:
            # Verified: The service call timed out.
            pass
