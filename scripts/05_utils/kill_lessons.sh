...
echo "Killing lesson processes (python + native + proxy)..."
...
# Kill the lifecycle proxy
pkill -9 -f "rosrustext_lifecycle_proxy" >/dev/null 2>&1 || true
...
echo "Killing rosbridge infrastructure..."
pkill -9 -f "rosbridge_websocket" >/dev/null 2>&1 || true
pkill -9 -f "rosapi_node" >/dev/null 2>&1 || true
