...
echo "=================================================="
echo "TEST SUITE: Rust/rclrs (Lessons 1-6)"
echo "=================================================="
...
start_node_bg "$LOG_BASE/l01.log" ros2 run lesson_01_node_rclrs lesson_01_node_rclrs
PID=$RET_PID
sleep 2
stop_node $PID
check_ret 0
...
start_node_bg "$LOG_BASE/l02.log" ros2 run lesson_02_publisher_rclrs lesson_02_publisher_rclrs
PID=$RET_PID
sleep 2
...
# Rclrs Sub
start_node_bg /dev/null ros2 run lesson_02_publisher_py lesson_02_publisher
PID_PUB=$RET_PID
RET=1
if wait_for_topic "/tutorial/chatter"; then
    start_node_bg "$LOG_BASE/l03.log" ros2 run lesson_03_subscriber_rclrs lesson_03_subscriber_rclrs
    PID_SUB=$RET_PID
    for ((i=0; i<10; i++)); do
        if [ -s "$LOG_BASE/l03.log" ]; then
             RET=0; break
        fi
        sleep 1
    done
...
start_node_bg "$LOG_BASE/l04.log" ros2 run lesson_04_service_rclrs service_server
PID_SRV=$RET_PID
RESP=""
if wait_for_service "/tutorial/compute_stats"; then
    for ((i=0; i<5; i++)); do
        RESP=$(timeout 20 ros2 service call /tutorial/compute_stats lesson_interfaces/srv/ComputeStats "{data: [1.0, 2.0, 3.0]}" 2>/dev/null)
...
start_node_bg "$LOG_BASE/l05.log" ros2 run lesson_05_parameters_rclrs lesson_05_publisher
PID=$RET_PID
VAL=""
if wait_for_node_params "/lesson_05_publisher"; then
    for ((i=0; i<5; i++)); do
        VAL=$(ros2 param get /lesson_05_publisher timer_period_s 2>/dev/null || echo "Fail")
        if [[ "$VAL" != "Fail" ]]; then break; fi
        sleep 1
    done
fi
stop_node $PID
if [[ "$VAL" != "Fail" && "$VAL" != "Timeout" ]]; then RET=0; else RET=1; fi
check_ret $RET
R5=$?

# --- Lesson 06: Orchestration ---
echo -n "[L06] Lifecycle Orch... "
start_node_bg "$LOG_BASE/l06_pub.log" ros2 run lesson_06_lifecycle_rclrs lesson_06_lifecycle_publisher
PID_PUB=$RET_PID
start_node_bg "$LOG_BASE/l06_sub.log" ros2 run lesson_06_lifecycle_rclrs lesson_06_lifecycle_subscriber
PID_SUB=$RET_PID
start_node_bg "$LOG_BASE/l06_mgr.log" ros2 run lesson_06_orchestration lifecycle_manager
PID_MGR=$RET_PID

sleep 5
# Case-insensitive check for LC state
